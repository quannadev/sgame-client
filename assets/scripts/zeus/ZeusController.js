let ZeusEvent = require('ZeusEvent');
let ZeusController = SmartFoxSDK.BaseController.extend({
    m_tableInfo: null,
    registerEvents(){
        this._super();
    ...
    onHistoryEvent(event){
        ...
        UIManger.show("UIZeusHistoryTransaction", {pop: true, src: 'zeus', data: {items: items}});
    },
    onLeaderBoardEvent(event){
        ...
        UIManger.show("UIZeusRank", {pop: true, src: 'zeus', data: {items: items}});
    },
    ...
    onUserResultEvent(event){
        let userBet = new ZeusEvent.ResultEvent().fromEvent(event);
...
let getInstance = function () {
    if(instance == null)
        instance = new ZeusController("UIZeus", "zeus");
    return instance;
}
