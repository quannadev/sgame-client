cc.Class({
    ...
    onEnable(){
        ...
        let dices = this.DiceAnimation.node.getComponent("AnimationEvent");
    ...
    setThangThua(){
        let maxThang = this.getMaxDayThang();
        let maxThua  = this.getMaxDayThang();
    ...
    showGame(isLoc, quyLoc) {
        if (isLoc != null)
            this.btnXinLoc.getComponent(cc.Button).interactable = isLoc;
    ...
    updateDataCau() {
        if(SmartFoxSDK.TaiXiuController.m_tableInfo){
            let itemCau = {};
            let listResult = SmartFoxSDK.TaiXiuController.m_tableInfo.result;
            if (cc.listCau.length > 0)
                itemCau.sessionId = parseInt(cc.listCau[cc.listCau.length-1].sessionId) + 1;
            else
                itemCau.sessionId = 1;
    ...
    updateListCau(){
        if(SmartFoxSDK.TaiXiuController.m_tableInfo){
            if ( this.CauNode != null){
                let listCau = cc.listCau;
                let sizeDot  = this.CauNode._children.length;
                if (listCau.length > 16)
                    listCau = listCau.slice(listCau.length-16, listCau.length);
                for(let i = 0; i < listCau.length;i++){
                    if(i < sizeDot){
                        let cau = this.CauNode._children[i];
                        let dataCau = listCau[i];
                ...
                let a1 = cc.fadeOut(0.2);
                let a2 = cc.fadeIn(0.2);
    ...
    eventSessionDetail(event, data) {
        mm.Loading.show();
        // cc.txCurrentSession      = event.target.txSessionId;
        UIManger.show("UITaiXiuSessionDetail", {pop: true, src: 'taixiu', data:  event.target.txSessionId});
    },
    updateDataSessionDetail(dataSession) {
        let ui = this.getUIFromName("UITaiXiuSessionDetail");
    ...
    eventSoiCau() {
        mm.Loading.show();
        UIManger.show("UITaiXiuSoiCau", {pop: true, src: 'taixiu', data: []});
    },
    updateTaiXiuSoiCau(listResult){
        let ui = this.getUIFromName("UITaiXiuSoiCau");
    ...
    runEffectRefundWin(delay, refundChip, moneyWin){
        let self = this;
        this.moneyWin.active  = true;
        this.moneyWin.y       = 0;
        this.moneyWin.opacity = 255;
    ...
    showResultTaiXiu() {
        if(SmartFoxSDK.TaiXiuController.m_tableInfo.result) {
            let number = 0;
            for (let i = 0; i < SmartFoxSDK.TaiXiuController.m_tableInfo.result.length; i++) {
    ...
    updateTimer(){
        if (this.Timer != null){
            ...
            if (this.controller != null && this.controller.m_tableInfo != null)
                this.quayBatNode.active = this.controller.m_tableInfo.isBetting && this._time >= 0 && this._time <= 30;
        }
    },
    _turnOnTime(time){
        if (this.Timer != null){
    ...
    clickDongY(event){
        ...
        if(SmartFoxSDK.TaiXiuController.m_tableInfo.isBetting){
            if (this._currentBet > 0){
                let betRequest = new TaiXiuRequest.BetRequest();
    ...
    clickHelper(event){
        this.show("UITaiXiuHelper", {pop: true, src: 'taixiu'});
    },

    clickHistory(event){
        mm.Loading.show();
        UIManger.show("UITaiXiuTransaction", {pop: true, src: 'taixiu', data: {items: []}});
    },
    updateDataHistory(items) {
        let ui = this.getUIFromName("UITaiXiuTransaction");
    ...
    clickRank(event){
        mm.Loading.show();
        UIManger.show("UITaiXiuRank", {pop: true, src: 'taixiu', data: {items: []}});
    },
    updateDataRank(items) {
        let ui = this.getUIFromName("UITaiXiuRank");
        if (ui != null && ui.name == "UITaiXiuRank"){
            ui.getComponent(ui.name).updateData(items);
        }
    },
    showNoti(time, message) {
    ...
    eventXinLoc() {
        let betRequest = new TaiXiuRequest.RutLocRequest();
        SmartFoxSDK.TaiXiuController.ZoneInstance.send(betRequest.toSRequest());
        this.btnXinLoc.getComponent(cc.Button).interactable = false;
    },
    eventTanLoc() {
        this.show("UITaiXiuTanLoc", {pop: true, src: 'taixiu'});
    },
    eventRankTanLoc() {
        this.show("UITaiXiuDuaTop", {pop: true, src: 'taixiu', data: "VinhDanh"});
    },
    eventHistoryTanLoc() {
        this.show("UITaiXiuDuaTop", {pop: true, src: 'taixiu', data: "LichSu"});
    },
    getMaxDayThang(){
    ...
    setMaxDayThua(dayThua){
        localStorage.setItem("f68_day_thua_max", dayThua);
    },
    showResultRutLoc(rutLocInfo) {
    ...
    showReward(wardInfo) {
        let msg = "Bạn đã nhận được phần thường sự kiện đua TOP \n Vị trí: "+wardInfo.top+"\n" + "Phần thưởng: "+Utils.addDotToNumber(wardInfo.reward);
