let WinGame     = require("SinbadWinGame");
let Helper      = require("Helper");
let reel        = require("SinbadReel");
let line        = require("SinbadLine");
...
cc.Class({
    ...
    updateJackpot(){
        let key = "sinbad"+cc.betLevel;
        if(SmartFoxSDK.SinbadController.ZoneInstance.mySelf){
            if(SmartFoxSDK.SinbadController.ZoneInstance.mySelf){
                let variableJackpot = SmartFoxSDK.SinbadController.ZoneInstance.mySelf.getVariable(key);
                if(variableJackpot){
    ...
    updateUserVariableSlot(event){
        ...
        if(event.changedVars.indexOf(key) >= 0){
            ...
            if(event.user.isItMe){
                let freeSpin    = SmartFoxSDK.SinbadController.ZoneInstance.mySelf.getVariable(key).value;
    ...
    runQuay() {
        if (this._isFreeTrial){
            ...
            this.playSpin();
            this.isSpin    = true;
            let betRequest = new SinbadRequest.BetRequest();
            this.showLineWin(false);
            ...
            SmartFoxSDK.SinbadController.ZoneInstance.send(betRequest.toSRequest());
            // sub money
            if (this.freeSpin < 1)
    ...
    eventStopQuay: function() {
        ...
        let request = new SinbadRequest.StopAutoPlayRequest();
    ...
    autoQuay() {
        if (this._isFreeTrial){
            ...
            if (this.freeSpin < 1){
                if (this.isAuto){
                    let request    = new SinbadRequest.AutoPlayRequest();
    ...
    showMainLine: function() {
        if (this.lineWin.length > 0){
            if (this.listMainLines.children[this.lineWin[this.posLineWin%this.totalLineWin]] != undefined)
                this.listMainLines.children[this.lineWin[this.posLineWin%this.totalLineWin]].getComponent("SinbadMainLine").offEf();
            this.posLineWin = (this.posLineWin+1)%this.totalLineWin;
            if (this.listMainLines.children[this.lineWin[this.posLineWin]] != undefined)
                this.listMainLines.children[this.lineWin[this.posLineWin]].getComponent("SinbadMainLine").onEf();
        }
    },
    runReels: function () {
        Promise.all(this.reels.map(function(reel, index) {
